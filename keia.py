...
# Inisialisasi State
# Memastikan st.session_state tidak diinisialisasi ulang setiap reruns jika sudah ada data
...
if st.session_state.show_analysis and st.session_state.data is not None:
    ...
    # Menghapus tombol "Lanjut" setelah analisis ditampilkan
    # (Tombol ini sudah tidak relevan karena show_analysis menjadi True di atas)
    ...
    with st.sidebar:
        ...
        if st.button("Hapus File & Unggah Baru", key="clear_file_btn", use_container_width=True, type="secondary"):
            ...
            st.experimental_set_query_params() # Membersihkan URL params jika ada
            st.rerun()
    ...
    try:
        ...
        filtered_df = df # Kembali ke data asli jika filter error
                ...
                if not filtered_df.empty:
                    try:
                        if chart["key"] == "sentiment": 
                            ...
                            data.columns = ['Sentiment', 'count'] # Pastikan nama kolom benar
                            fig = px.pie(data, names='Sentiment', values='count', title='Distribusi Sentimen')
                        elif chart["key"] == "trend": 
                            # Pastikan kolom 'Date' adalah tanggal dan Engagements adalah angka
                        ...
                        st.warning(f"Gagal membuat grafik {chart['title']}: {e}")
                        fig = None # Pastikan fig adalah None jika ada error
